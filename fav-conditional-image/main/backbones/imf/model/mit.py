...
class RoPEAttention(nn.Module):
    ...
    def forward(self, x, rope_freqs):
        ...
        # manual attention to match the JAX implementation
...
class MiT(nn.Module):
    """MeanFlow improved Transformer: a shared backbone with u/v heads of depth aux_head_depth."""
    ...
    def __init__(
        ...
        # v heads are unused during evaluation
    ...
    def _build_sequence(self, x, h, w, t_min, t_max, y):
        """Prepend conditioning tokens (time/omega/cfg/class) to patch embeddings."""
    ...
    def forward(self, x, t, h, w, t_min, t_max, y):
        """Returns predicted (u, v): average and instantaneous velocity fields."""
        # condition only on h = t - r, not t (arxiv 2502.13129)
...
def inject_lora(model, rank=4, alpha=None, target_modules=None, dropout=0.0):
    """Inject LoRA into MiT attention projections. Returns (model, n_trainable, n_total)."""
    ...
    for name, module in model.named_modules():
        if isinstance(module, TorchLinear):
            ...
            if attr_name in target_modules:
                original_linear = module._flax_linear
                module._flax_linear = LoRALinear(
                    original_linear, rank=rank, alpha=alpha, dropout=dropout
                )
