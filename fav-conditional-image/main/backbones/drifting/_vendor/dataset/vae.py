...
def vae_enc_decode(replicate_params: bool = True):
    ...
    if replicate_params:
        ...
        def _replicate(x):
            # FlaxAutoencoderKL.from_pretrained returns jax.Arrays already on
            # CpuDevice (unpinned_host). device_put-ing those directly to a
            # CUDA NamedSharding leaves stale device metadata, which trips
            # PJRT lookup ("PjRtDevice not found: cuda:0") during JIT constant
            # folding. Coerce through np.asarray first to detach.
            x = np.asarray(x)
