...
#         # x = (x * 0.5 + 0.5).clamp(0,1).mul(255) #.to(torch.uint8)
    #     return x
    def decode_latents_to_pixels(self, x, enable_grad=False): 
        self.init(x.device)
        
        # 외부에서 넣어준 enable_grad 값에 따라 컨텍스트 결정
    ...
    def decode(self, x, enable_grad=False): # final latents => raw pixels
        if x.shape[1] == 2 * 4:
            mean, std = x.to(torch.float32).chunk(2, dim=1)
            x = mean + torch.randn_like(mean) * std
            return self.decode_latents_to_pixels(x, enable_grad=enable_grad)
        elif x.shape[1] == 4:
            return self.decode_latents_to_pixels(x, enable_grad=enable_grad)
        elif x.shape[1] == 3:
            return x
